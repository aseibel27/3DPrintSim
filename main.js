...
async function mainFunction() {
    ...
    // Usage
    let myGeometry;
    // myGeometry = await loadSTLFile('Eevee.stl');
    myGeometry = await loadSTLFile('20mm_calibration_cube.stl');
    myGeometry.scale(5,5,5);
    const meshMaterial = new THREE.MeshBasicMaterial({ color: 0xff5533 });
    const myMesh = new THREE.Mesh(myGeometry, meshMaterial);
    centerObject(myMesh, overlayGridHelper);
    overlayScene.add(myMesh);
    console.log(myMesh);
    ...
    centerObject(cube, overlayGridHelper);
    ...
    for (let i = 0; i < sortedPairs.length; i++) {
        if (!currentPosition.equals(sortedPairs[i].v0)) {
            ...
            addMoveAndObject(currentPosition, sortedPairs[i].v0, 500, false);
